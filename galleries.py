...
def extractNdetectionPerId(galley, N):
    galleryPerPeopleId = {}
    ...
    for peopleid in galleryPerPeopleId.keys():
        addedIdPerPerson = 0
        assert len(galleryPerPeopleId[peopleid]) >= N
        while(addedIdPerPerson < N):
            indexToBeAdded = random.choice(galleryPerPeopleId[peopleid])
            if( indexToBeAdded not in galleryIndicesMVM):
                galleryIndicesMVM.append(indexToBeAdded)
                addedIdPerPerson += 1
